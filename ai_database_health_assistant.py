...
if mode == "Demo Database":
    ...
    if connect_button:
        ...
        try:
            ...
            query_store_status = get_query_store_status(conn)
            if str(query_store_status.get("Actual State", "")).upper() in ["READ_WRITE", "READ_ONLY"]:
                query_store_history = get_query_store_history(conn, days=7)
            else:
                query_store_history = pd.DataFrame(columns=[
                    "Interval Start", "Query ID", "Plan ID", "Query",
                    "Executions", "Avg Duration ms", "Avg CPU ms", "Avg Logical Reads"
                ])
            ...
            st.session_state.db_analysis_cache = {
                ...
                "blocking": blocking,
                "growth": growth,
                "query_store_status": query_store_status,
    ...
    query_store_status = cached.get("query_store_status", {
    ...
    query_store_history = cached.get(
        "query_store_history",
